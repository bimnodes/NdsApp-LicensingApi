-- Fix monthly subscription valid_until fallback.
--
-- Reason:
-- Stripe subscription webhook events can arrive without current_period_end.
-- The previous fallback always used now() + interval '1 year', which made monthly
-- PRO_MONTHLY_10 licenses valid for one year instead of one month.
--
-- This migration makes the fallback depend on nds_plans.billing_interval.
