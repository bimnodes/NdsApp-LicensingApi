...
public sealed class SupabaseLicenseKeyIssuanceService : ILicenseKeyIssuanceService
{
    ...
    public SupabaseLicenseKeyIssuanceService(HttpClient httpClient, IOptions<SupabaseOptions> options)
    {
    ...
    public async Task<JsonElement> IssueAsync(string email, CancellationToken cancellationToken)
    {
        ...
        using var response = await _httpClient.PostAsJsonAsync(
            "/rest/v1/rpc/nds_issue_license_key_for_email",
            payload,
            JsonOptions,
            cancellationToken);
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new SupabaseRpcException(
                "Supabase RPC 'nds_issue_license_key_for_email' failed.",
